...
- `site.config.json` — master switchboard. **Colour and typography are independent axes**: `theme.preset` picks a palette from `theme.presets` (**22 presets**; default `parchment.harbour.blue`). The palettes were sampled from well-known editor colour schemes but **deliberately renamed to original what3words-style dotted triads** — `blue.hour.rooftop`, `burnt.amber.workshop`, `parchment.harbour.blue` etc. — because Yash did not want herdr.dev's names copied. Preset keys are the display names; don't reintroduce the upstream names. and `theme.style` picks a typography/surface treatment from `theme.styles` (`terminal` default, `modern`). Each palette is 8 fields (`appearance` + 7 colours); both axes are applied as CSS custom properties at runtime by `themeVariables()` in `src/lib/content.ts`. Also holds `sections` show/hide toggles, `features` (commandPalette, particles, mouseGlow, themePicker), `meta`.
