        ...
        {writing.items.map((post) => (
          <li key={post.slug}>
            <Link
              ...
              className="rounded-theme group block border border-transparent p-4 transition-colors hover:border-line hover:bg-panel"
            >
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:gap-6">
                <p className="shrink-0 font-mono text-xs text-muted">{post.date}</p>
                <div>
                  <h3 className="heading text-ink transition-colors group-hover:text-accent">
                    {post.title}
                  </h3>
